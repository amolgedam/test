<?php
  ...
  class ManageCashOrder extends CI_Controller {
    ...
    public function index($flag='')
  {
    ...
    $this->load->view('managecashorder/list',$data);
  ...
  public function ajax_manage_page()
    {
       ...
       $emp_id = $_POST['SearchData5'];
        ...
        $cond="u.created_by='".$_SESSION['admin']['id']."'";

        if(!empty($Filter))
        {
            $cond.=" and dwd.date='".$Filter."'";
        }
        if(!empty($emp_id))
        {
            $cond.=" and dwd.emp_id='".$emp_id."'";
        }

        //print_r($cond);exit;
        
	     $ManageData = $this->ManageCashOrder_model->get_datatables($cond);
        ...
        foreach ($ManageData as $row) 
        {
            ...
            $nestedData = array();
            $nestedData[] = $no;
            $nestedData[] =  ucfirst($row->cust_name);
            $nestedData[] = $row->emp_name;
            $nestedData[] = $row->address;
            $nestedData[] = $address;
            $nestedData[] = $row->quantity.'-Liter';
        ...
        $output = array(
                    "draw" => $_POST['draw'],
                    "recordsTotal" => $this->ManageCashOrder_model->count_all($cond),
                    "recordsFiltered" => $this->ManageCashOrder_model->count_filtered($cond),
                    "data" => $data,
                );
