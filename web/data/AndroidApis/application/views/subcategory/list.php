            ...
            <div class="col-lg-12">
                <div class="box box-primary">
                    ...
                    <div class="box-header">
                      ...
                      <div class="col-md-4">
                        <label>Product</label>
                        <select id="product" name="product" class="form-control filter_search_data4">
                          <option value="All">All</option>
                          <?php if(!empty($cotegory)) 
                          { 
                            foreach ($cotegory as $key) 
                            {      
                            ?>
                      ...
                      <div class="col-md-4">
                        <div class="content-header_button  pull-right"> 
                          ...
                          <a class="btn btn-primary" title="Create" href="<?= $createAction ?>">Add Product</a> 
                          <a class="btn btn-success" title="Active" href="<?= $StatusActive ?>">Active All</a> 
                          <a class="btn btn-danger" title="Active" href="<?= $StatusInactive ?>">Inactive All</a> 
                        </div>  
                    </div>
                    </div>
                    </div>
                        ...
                        <table id="table" class="table table-bordered table-striped example_datatable">
                            <thead>
                                <tr>
                                    ...
                                    <th>Category</th>
                                    <th>Product name</th>
                                    <th>One Liter Price</th>
                                    <th>Half Liter Price</th>
                                    <th>Status</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
...
<script>
    var url = '<?= site_url('Subcategory/ajax_manage_page')?>';
    var actioncolumn=7;
</script>
  ...
  function Delete(cid)
{
  ...
  { 
    ...
    $.ajax({
        type:"POST",
        url:"<?php echo site_url('Subcategory/delete'); ?>",
        data:datastring,
        cache:false,        
...
{ 
  ...
  if(cnf==true)
  {
    ...
    $.ajax({
        type:"POST",
        url:"<?=  site_url('Subcategory/change_status')?>",
        data:datastring,
        cache:false,                    
        success:function(returndata)
        {
