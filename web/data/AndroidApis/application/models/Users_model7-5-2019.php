...
class Users_model extends CI_Model
{
  ...
  var $column_order = array(null,'us.name','us.email','us.mobile','us.business_type','us.login_type','us.status',null); //set column field database for datatable orderable
        ...
        if($_POST['search']['value']) // if datatable send POST for search
            {
                ...
                {  
                    $cond  = "";
                    $cond.=" ( us.name LIKE '%".trim($show_string)."%' ";
                    $cond.=" OR us.email LIKE '%".trim($show_string)."%' ";
                    $cond.=" OR us.mobile LIKE '%".trim($show_string)."%' ";
                    $cond.=" OR us.business_type LIKE '%".trim($show_string)."%' ";
