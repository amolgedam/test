...
{   
   ...
   //print_r("sd");exit();
    ...
    function getpurchaseData($branch_id)
    {
     ...
     function getuserdata($cond)
    {
        $this->db->select("a.*,d.designation_name");
    ...
    function get_employeedata($condition)
    {
        $this->db->select('a.*,ad.name');
        $this->db->from('attendence a');
        $this->db->join('admin ad','ad.id=a.emp_id','left');
        $this->db->where($condition);
