...
<script>
    var url = '<?= site_url('Demo_details/ajax_manage_page')?>';
    var actioncolumn=6;
</script>
...
function Delete(obj,id)
{
  ...
  { 
    ...
    $.ajax({
        type:"POST",
        url:"<?php echo site_url('Demo_details/delete'); ?>",
        data:datastring,
        cache:false,        
...
{ 
  ...
  if(cnf==true)
  {
    ...
    $.ajax({
        type:"POST",
        url:"<?=  site_url('Demo_details/change_status')?>",
        data:datastring,
        cache:false,                    
        success:function(returndata)
        {
